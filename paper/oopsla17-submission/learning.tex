...
labels $\B$.
%
Once trained, we can $\evalsym$uate a $\Model$ on a new input vector,
producing the confidence $\Runit$ that the blame label should apply. % be
%applied.
%
We describe multiple $\Model$s and their instantiations of
$\trainsym$ and $\evalsym$ (\autoref{sec:models}).
