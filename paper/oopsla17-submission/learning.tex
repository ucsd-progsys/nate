...
(4) neural networks.
%
For each algorithm, we describe the model it learns, the training
procedure, any hyper-parameters that must be tuned by a human, and a
...
%
Standard algorithms for training decision trees include C4.5 \ES{CITE} and
CART \ES{CITE}.
%
...
to make the final prediction \ES{CITE?}.
%
Random forests \ES{CITE} are an instance of this method, with decision
...
defined as $f(x) = \mathsf{max}(0,x)$ \ES{CITE}.
