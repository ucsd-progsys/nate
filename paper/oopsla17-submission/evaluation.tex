\mysection{Quantitative Evaluation}
...
%
\begin{itemize}
\item \textbf{How accurate is \toolname?}
  %
  ...
  % of the error?
\item \textbf{What features are used by \toolname?}
  %
  Which program features are the most
  \emph{most important} for
  localizing errors?
   (\autoref{sec:feature-utility})
  % How much do the features described in \autoref{sec:features}
  % contribute to our predictions?
\item[Why does \toolname work? (\autoref{sec:qualitative})]
  %
  Do the models learned by \toolname
  correspond to our intuition about
  the real causes of errors? That is,
  can we \emph{interpret} the individual
  predictions (correct or not)?
\end{itemize}
%\ES{may want to swap (1) and (2)..}


1. MLP-500 (fancy) is 72% vs Sherrloc 56% for top-1
2. Logistic (simple) is 60% vs Sherrloc 56% for top-1
3. MLP-500 reaches 91% for top-3
4. InSlice is crucial, just InSlice + LocalSyntax gets Logistic 10pts higher than OCaml, on par with Sherrloc
5. Typing features provide more bang/buck than contextual syntax
6. Tree learns some simple and intuitive rules for predicting no-blame (e.g. defining a function is a fine thing to do, calling a function likewise)
7.
