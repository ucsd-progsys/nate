...
features to be relatively unimportant,
which would support the hypothesis that
the context's \emph{type} nicely
summarizes the properties of the
surrounding expressions.
%
Finally, we found that the \emph{size}
of the sub-expression was not a useful
feature. This was unexpected as we thought
...
predictions made by the \emph{decision tree}
based models, we found that the models
do indeed capture some simple and intuitive
rules for predicting well-typededness.
%
For example, in an application, if the
left argument is a function then the
error is likely on the right sub-term;
in a function definition \RJ{fixme: orig
