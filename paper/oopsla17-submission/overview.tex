...
of different kinds of ways to
represent programs, from raw
...
\eg code completion \cite{Devanbu:2012,Raychev:2014}
or decompilation \cite{Raychev2015-jg,Bielik2016-br}
...
abstract interpretation~\cite{CousotCousot77}.
%
...
neighbors.
%
For example, \eg a |+| operator tells
...
expressions, their inferred types
(when available).
%
However, in we have found that
the most important contextual
signal is whether or not the
expression occurs in a
\emph{minimal type error slice}~\citep{Haack2003-vc}
...
one of them is the real location
