...
\begin{ecode}
  let rec sumList xs =
    match xs with
    | []   -> (*@\colorbox{tree!75}{\colorbox{sherrloc!75}{[]}}@*)
    | h::t -> h (*@\colorbox{tree!75}{+}@*) (*@\colorbox{fix!75}{sumList t}@*)
\end{ecode}
\end{minipage}
...
\caption{(left) An ill-typed \ocaml program that should sum the elements of a
  ...
  %
  (1) the \colorbox{fix!75}{\ocaml} compiler;
  %
  (2) the \colorbox{sherrloc!75}{fix} made by the programmer; and
  %
  (3) \colorbox{tree!75}{minimizing} the number of edits required.
