...
$\sim$10\% of programs where \emph{none} of our Top-3 predictions are correct.
...
\end{ecode}
%
Leaving aside issues of \emph{functional} correctness, the error in the
student's code is that she has swapped function and argument in the
application |z a|.
%
\sherrloc correctly blames the use of |z|, while our classifier
(incorrectly) blames the entire application (\ocaml blames the use of
|f| on line 4).
\ES{TODO: WHY??}
%

\paragraph{A Functional \texttt{while}-loop}
% data/sp14/0255.ml
Finally, consider the following attempt at the functional
\texttt{while}-loop from before.
%
\begin{ecode}
  let rec wwhile (f, b) =
    let check = f b in
    ...
    if __y__ = false then
      ...
      wwhile (f, ==x==)
\end{ecode}
% ES: the let (x, y) = (f, b) is actually a match, but this seems easier to present..
Our student

\ES{marking ``y'' seems totally bogus, why is it even part of the error
  slice? (other locations are ``f b'' and ``false'' though, which is not
  any better)}
